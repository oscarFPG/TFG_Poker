...
public class Three_of_a_Kind extends Play {
         ...
         for (int i = 0; i < c.length - 1; i++){
            if (ok(c[i], c[i+1])) {
                ...
                if (it == 2){
                    ...
                    c[i].setSol(true);
                    c[i+1].setSol(true);
                    this.name += NumbertoString(highThreeCard) + " (" + cardstoString(c) + ")";

                    for (int j = 0; j < c.length; ++j) {
                        if (highFourCard == 'X' && c[j].getNumber() != highThreeCard) {
                            highFourCard = c[j].getNumber();
                        }
                        else if (highFourCard != 'X' && highFiveCard == 'X' && c[j].getNumber() != highThreeCard) {
                            highFiveCard = c[j].getNumber();
                            break;
                        }
                    }
                    return true;
    ...
    public char[] getHighCard() {
        char[] hc = {highThreeCard, highFourCard, highFiveCard};
        return hc;
    }
